...
1. 点击右上角的保存按钮![](/images/save.png)，保存你的活动记录（保存为当前文件夹下的`{用户id}-warmup-{你所在的组别}.json`，例如`a94c-warmup-A.json`）
2. 停止录屏
3. 告知实验负责人

在完成**所有任务后**，请打包每个任务的 `{用户id}-warmup-{你所在的组别}.json` 文件和 录屏文件，提交给实验负责人。
